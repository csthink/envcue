...
- [x] T6 集成与分发(真机回归 + Homebrew)→ **M3 达成**
  - [x] T6.1 真机回归:spec §8 八条全过(隔离脚本验 6 条 + 真机 `~/.zshrc` install 幂等 + GUI 肉眼);**含 PROPOSAL-004 修复**(切换时差量 `unset` 旧 scene 残留变量,差量集 == diff.removed)
  - [x] T6.2 打包 + 分发链:`scripts/package-app.sh`(组装 `.app` bundle)、`packaging/envcue.rb.tmpl`(源码构建 formula:装 .app + symlink `envcue` 上 PATH)、`.github/workflows/release.yml`(v* tag → 算 sha → 渲染 → 推 `csthink/homebrew-tap`);**formula 已本地 `brew install` 端到端验证通过**
  - [x] M3 收尾:README 转双语(英文 `README.md` 默认 + `README.zh-CN.md` + 顶部互链 + `brew install csthink/tap/envcue` 安装段)
  - [x] 发版核验:`v0.1.0` 已发(release CI 推 formula 到 `csthink/homebrew-tap`),真机 `brew install csthink/tap/envcue` 装好可用、`envcue --help`/`status` 正常 → **M3 达成**
  - [ ] T6.3 签名公证:仍在决策闸门后(v1 默认不做,未签名 formula 不触发 Gatekeeper)
