...
- **Commit 语言 + 署名**:commit message 一律英文,`<type/Tn>: <summary>` 形式。**任何 commit message 与 PR body 绝不出现任何形式的 AI 署名 / 协作标记**(不写 `Co-Authored-By`、不写 `Generated with Claude`、不提 Claude/Anthropic/AI)。
...
- [ ] T0 工程骨架(Swift Package 5 target + 双模式入口)
- [ ] T1 EnvCueCore(模型 + 求值 + diff + 序列化 + generation)  ← **安全门 G1**(T1.5/T1.6)
- [ ] T2 EnvCueKeychain(generic password 读写 + keychain-get stdout)
- [ ] T3 EnvCueShell(原子写 + shim 幂等 + precmd hook 模板)  ← **安全门 G2**(T2+T3)
- [ ] T4 EnvCueCLI(子命令装配,走单写者 apply 路径)  ← **安全门 G3**(端到端无泄露)→ **M1**
- [ ] T5 EnvCueApp(MenuBarExtra + Liquid Glass)  ← **视觉门** → **M2**
- [ ] T6 集成与分发(真机回归 + Homebrew)→ **M3**
  - [ ] M3 收尾:README 转双语 —— 现有中文 `README.md` 迁到 `README.zh-CN.md`,新写**英文** `README.md`(GitHub 默认展示),两份顶部互加语言切换链接;同步补上可用的 `brew install mars/tap/envcue` 安装段
